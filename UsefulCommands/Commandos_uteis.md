...
```ruby
 ...
 # Ou
 FOR %P IN (172.25.131 172.25.130 172.25.255 172.30.27 172.30.31 172.30.32 172.30.36 172.30.42 172.30.43 172.30.45 172.30.46 192.168.77 10.10.64) DO @(FOR /L %i IN (1,1,254) DO @(start "" /b cmd /c "ping -w 1 -n 1 %P.%i 2>NUL | findstr /I "TTL=12" 1>NUL 2>NUL && echo Address: %P.%i && query user /server:%P.%i 2>NUL | FINDSTR myusername"))
```
