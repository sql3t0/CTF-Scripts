...
for dir in $(find / -maxdepth 2 -type d 2>/dev/null | egrep -v "/proc|/lib|/bin|/sbin|/lib*|/boot|/mnt|^/$" | egrep -v "^/\w+$")
    do
        ...
        PatternDBs="new PDO\(.*\);"
        RegexFull="([\"\'\[\(\{\$]?)($PatternUser|$PatternPass|$PatternMixx)([\"\'\]\)\}\$]?)([ ]?)([:=])([ ]?)([\"\'\[\(\{])(.*)([\"\'\]\)\}])|([<])($PatternUser|$PatternPass|$PatternMixx)([>])(.+)([\/>])|$PatternDBs"
        filename=$(egrep --exclude-dir={lib,bin,sbin,boot,mnt,proc} -irl "$RegexFull" $dir 2>/dev/null)
        if [ ! -n "$filename" ]
            then
                printf "\r[+] Hostname: `hostname`  , Folder: $dir"
            else
                printf "\r[+] Hostname: `hostname`  , Folder: $dir \n";
                egrep --color=always -aino "([\"\'\[\(\{\$]?)($PatternUser|$PatternPass|$PatternMixx)([\"\'\]\)\}\$]?)([ ]?)([:=])([ ]?)([\"\'\[\(\{])(.*)([\"\'\]\)\}])|([<])($PatternUser|$PatternPass|$PatternMixx)([>])(.+)([\/>])|$PatternDBs" $filename 2>/dev/null | cut -b 1-300 | uniq &
            fi    
    done
